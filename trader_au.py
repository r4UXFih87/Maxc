...
def executar_trading():
    while True:
        ...
        if saldo_usdc >= 1 and media_rapida > media_devagar:
            print("COMPRA")
            conta.new_order(symbol="PEPEUSDC", side=Side.BUY, order_type=OrderType.MARKET, quote_order_quantity=saldo_usdc)
        else:
            print("Ordem não aplicada para compra")

        if saldo_pepe >= pepe and media_rapida < media_devagar:
            print("VENDA")
            conta.new_order(symbol="PEPEUSDC", side=Side.SELL, order_type=OrderType.MARKET, quote_order_quantity=saldo_pepe)
        else:
